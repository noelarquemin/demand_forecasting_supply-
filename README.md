...
Use direct multi-step forecasting: train separate model (or separate head) per horizon (j+1, j+7, j+14). Evaluate each independently.
...
for SKU_ID in group A --> 1.65 (95 %)
for SKU_ID in group B --> 1.28 (90 %)
for SKU_ID in group C --> 1.06 (85 %)
...
if onhand <= reorder_point --> initiate_order of 'Order_Quantity' = reorder_quantity
...
- Baseline (naïve, moving average, dataset forecast)
